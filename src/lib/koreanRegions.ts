// 시/도 → 시/군/구 지역 선택용 정적 데이터. 회원가입/업체 등록 주소는 다음(카카오)
// 우편번호 검색으로 입력받지만, AI 견적 매칭의 "지역 기준"은 정확한 주소가 아니라
// 넓은 범위 매칭용이라 이 목록에서 직접 고르게 한다.
export type KoreanRegion = {
  name: string; // "서울" 같은 짧은 표기. Company.region 문자열에 항상 포함되는 형태라
  // "구/군/시 미선택(전체)" 시 이 값 자체를 매칭 조건으로 사용한다.
  districts: string[];
};
...
export const KOREAN_REGIONS: KoreanRegion[] = [
  ...
  { name: "광주", districts: ["광산구", "남구", "동구", "북구", "서구"] },
  { name: "대전", districts: ["대덕구", "동구", "서구", "유성구", "중구"] },
  { name: "울산", districts: ["남구", "동구", "북구", "중구", "울주군"] },
  { name: "세종", districts: [] },
  {
    name: "경기",
    districts: [
